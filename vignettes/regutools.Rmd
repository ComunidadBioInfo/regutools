...
author:
  ...
  - name: Jesus Emiliano Sotelo Fonseca
    affiliation:
    - LANGEBIO, Cinvestav, Irapuato, Guanajuato, MX
...
```{r}
head(list_attributes(ecoli_regulondb, "GENE", description=TRUE), 8)
```

# Retrieving data

Since the *regulondb* object is an extension of the *SQLiteConnection*, users can retrieve data from a *regulondb* object using the function `dbGetQuery()`. Additionally, this package provides a wrapper function to build queries to the database. This function is called `get_dataset()` and has a very similar syntax to the `getBM()` function from the `r BiocStyle::Biocpkg("biomaRt")` package. The main arguments of the `get_dataset()` function are a *regulondb* object, a dataset (or table) of the database, the fields of the dataset to retrieve (i.e. the attributes) and filters to specify what information to get. The code below shows an example where four attributes from the dataset **"GENE"** for the genes *araC*, *crp* and *lacI* are retrieved. Note that if the `filters=` parameter is empty, the function will retrieve all the hits it find in the database.  
...
Some of the filters, such as *posright* or *posleft*, can be filtered by a interval range. For example, the code below filters the search to the genes where the start positions, *posright*, lie between position 1 and position 5000 of the genome. The parameter `inverval=` is used to specify that the filter for that field will be defined by an interval rather than a exact match.
...
By default, the function `get_dataset()` outputs a **regulondb_result** object, which is an extension of a **DataFrame** that stores additional metadata. These metadata includes information of the organism name, the database and genome versions, and the table (or dataset) of the *regulondb* object that was queried by `get_dataset()`.
...
To enable integration with other Bioconductor packages, we provide the function `convert_to_granges()` which converts a **regulondb_result** object into a **GRanges** object. For example, the result stored in the variable `res` has genomic coordinates and it can be represented as a **GRanges** object. 
...
In a similar manner, the function `convert_to_biostrings()` converts **regulondb** objects into objects from the `r BiocStyle::Biocpkg("Biostrings")` package. Possible outputs of `convert_to_biostrings()` are **DNAStringSet** objects if the parameter `seq_type="DNA"` or a **BStringSet** if the parameter `seq_type="product"`. 
...
It is also possible for the output of `get_dataset()` to be a **DNAStringSet** object by specifying the parameter `output_format="DNAStringSet"` or a **BStringSet** object by specifying `output_format="BStringSet"`. Note that the functions to convert **regulondb_result** objects will throw errors if there is insufficient information for the coersion to occur. For example, the function `convert_to_granges` will throw an error if the genomic coordinates are missing from the **regulondb_result** object.

# Building complex queries

In the *regutools* package, we have implemented functions to write user-defined queries programmatically. We have implemented the possibility of filtering results by partial matching, filtering by numeric intervals, and filtering by several attributes of the database.
...
For example, the code below extracts the genes where either `name` or `product_name` contain the word *Ara*, if the gene is in the forward strand or if the right position of the gene is between 2000 and 40000000. 
...
Software tools such as [Cytoscape](https://cytoscape.org/index.html) are useful to interactively explore networks. The function `get_regulatory_network()` has a parameter `cytograph=` that if set to `TRUE`, it will visualize the network in a cytoscape session. Of note, this feature will only work if when users have a Cytoscape session open in their computer. 
